...
class HFTransformersEngine(InferenceEngine):
    ...
    def generate_stream(self, prompt: str, max_new_tokens: int = 256):
        """
        Yields text chunks as they're produced. Runs model.generate() in a
        background thread writing into a TextIteratorStreamer queue, while
        this generator reads from that queue -- the standard HF pattern for
        streaming, and what the FastAPI /generate/stream endpoint consumes.
        """
